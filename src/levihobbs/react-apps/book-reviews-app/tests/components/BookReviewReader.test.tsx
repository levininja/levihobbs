...
const mockBookReviewWithoutCover: BookReview = {
  ...
  titleByAuthor: 'Brave New World by Aldous Huxley',
  coverImageId: null,
  coverImageUrl: null
};
...
describe('BookReviewReader - Test Suite', () => {
  ...
  describe('Modal Overlay Behavior', () => {
    it('should render as a full-screen overlay that hides the rest of the app', () => {
      ...
      // Should have overlay styling (position fixed, full screen, slight transparency)
  ...
  describe('Book Cover Image Display', () => {
    it('should display the book cover image at the top of the reader', () => {
      render(<BookReviewReader bookReview={mockBookReview} onClose={mockOnClose} />);
      
      const coverImage = screen.getByAltText('Cover of 1984');
      expect(coverImage).toBeInTheDocument();
      expect(coverImage).toHaveAttribute('src', 'https://example.com/1984-cover.jpg');
    });
    ...
    it('should use backup image when no cover image is provided', () => {
      ...
      const coverImage = screen.getByAltText('Cover of Brave New World');
      expect(coverImage).toBeInTheDocument();
  ...
  describe('Book Metadata Display', () => {
    it('should display author, publish date, my rating, and bookshelves in a side section', () => {
      ...
      expect(screen.getByText('Science Fiction Classics')).toBeInTheDocument();
    });
  ...
  describe('Review Content Display', () => {
    ...
    it('should left-align the review text', () => {
      ...
      const reviewText = screen.getByTestId('review-text');
  ...
  describe('Close Functionality', () => {
    ...
    it('should not crash when onClose is not provided', () => {
      render(<BookReviewReader bookReview={mockBookReview} />);
  ...
  describe('Layout and Styling', () => {
    it('should have proper semantic HTML structure', () => {
      ...
      expect(screen.getByRole('heading', { level: 1 })).toHaveTextContent('1984');
      expect(screen.getByRole('heading', { level: 2 })).toHaveTextContent('by George Orwell');
    });

    it('should handle bookshelves with and without display names', () => {
      const bookReviewWithMixedShelves: BookReview = {
        ...mockBookReview,
        bookshelves: [
          { id: 1, name: 'favorites', displayName: 'Favorites' },
          { id: 2, name: 'sf-classics' } // No displayName
        ]
      };
      ...
      expect(screen.getByText('Favorites')).toBeInTheDocument();
      expect(screen.getByText('sf-classics')).toBeInTheDocument();
    });

    it('should handle book review without publication year', () => {
